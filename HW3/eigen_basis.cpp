...
// Final Graph is information overload, feel free to rerun with lines commented out.
// 
// b = 1 and D=20 is nearly exact.
// Higher dimensions converge onto the correct answer, this should be true for pretty
// much any good basis set, so that's a good sign.
//
// The "bump" of the wave fucntion will be reproduced first, so choosing b based on on
// a D=1 result is a very good start. This is because the basis functions start localized
// and then become more diffuse, meaning that the tail will be produce really only in
// higher dimension expansions, as it is further from the origin. At high b, you may get
// the tail, but it will be a waste of the behavior you expect from the first basis function,
// as it will be better utulized capturing the predominant gaussian-esque bump. (We start our 
// expansion at n=1, not n=0. Our radial functions force psi to be 0 at r=0, essentially only 
// the odd harmonic functions (ish, they are behaviorly close at least.), but this means that 
// only the first dimension has a gaussian with a tail that is always positve, meaning it should
// ideally be the closest to the wave function we want, not that we know we wanted that without doing 
// hard maths first, but the idea is still useful for learning.)
//
///******************************************************************
...
main ()
{
  ...
  for (int i = 0; i < dimension; i++)
      ...
      for (int j = 0; j <= i; j++)
	{
	  ...
	  if (i == j)
	    gsl_matrix_set (Hmat_ptr, j, i, gsl_matrix_get (Hmat_ptr, i, j));	// Because sym
	  // print statement for debugging 
	  cout << "i = " << i << ", j = " << j
	    << ", Hij = " << gsl_matrix_get (Hmat_ptr, i, j) << endl;
	}
    }
  ...
  cin >> filename;
  ofstream out (filename);	// open the output file

  cout << endl;			// Uncomment to allow for chosing the state, however, exact wavefunction isn't designed for that.
  int target = 0;
//  cout << "Enter the state you wish to analyze ( 1 -> size): ";
//  cin >> target-1;

  gsl_matrix_get_col (eigenvector_ptr, Eigvec_ptr, target);

  double rmin = 0;
  double rmax = 10;
  double delta_r = 0.05;
  int size = ((rmax - rmin) / delta_r) + 1;
  double r_array[size] = { 0. };
  double wfbasis[size] = { 0. };
  double wfexact[size] = { 0. };
  // NORMALIZATION MAKES THIS LESS TRIVIAL THAN USUAL
  for (int i = 0; i < size; i++)
	...
	{
	  wfbasis[i] +=
	    (gsl_vector_get (eigenvector_ptr, j) *
	     ho_radial (j + 1, 0, b_ho, r));
	}
      wfexact[i] = ho_radial (2, 0, b_ho, r);
//wfexact[i] = 2.*r*exp(-r);
    }
...
// NOW NORMALIZE
  ...
  double basis_area = 0;
  double exact_area = 0;	//Numpy notation would make this easier
  basis_area +=
    (wfbasis[0] * wfbasis[0] + wfbasis[size - 1] * wfbasis[size - 1]) / 2.;
  for (int i = 1; i < size - 1; i++)
    basis_area += wfbasis[i] * wfbasis[i];
  basis_area *= delta_r;
  for (int i = 0; i < size; i++)
    wfbasis[i] /= sqrt (basis_area);
  exact_area +=
    (wfexact[0] * wfexact[0] + wfexact[size - 1] * wfexact[size - 1]) / 2.;
  for (int i = 1; i < size - 1; i++)
    exact_area += wfexact[i] * wfexact[i];
  exact_area *= delta_r;
  for (int i = 0; i < size; i++)
    wfexact[i] /= sqrt (exact_area);

  for (int i = 0; i < size; i++)
    {
      out << scientific << setprecision (8)
	<< r_array[i] << "   " << wfbasis[i] << "   " << wfexact[i] << endl;
