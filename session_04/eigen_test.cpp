...
//  double eigenvalue;          // one of the eigenvalues of the matrix 
  ...
  for (int dimension = 1; dimension < 2000; dimension *= 2)
    {
...
//  eigenvalue = 0;
 ...
 << " seconds for " << dimension << " dimensions\n\n";
	
out << scientific << setprecision (8)
	    << log10 (dimension) << "   "
	    << log10 ((double) (end - start) / (double) CLOCKS_PER_SEC)
	    << endl;
...
//    }
	    ...
	    gsl_vector_free (eigenvector_ptr); gsl_eigen_symmv_free (worksp);}
