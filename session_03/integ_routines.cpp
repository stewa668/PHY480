...
// Integration using Simpson's rule
float three_eigths_rule ( int num_pts, float x_min, float x_max,
                      float (*integrand) (float x) )
{
   float interval = ((x_max - x_min)/float(num_pts - 1));  // called h in notes
   float sum=  0.;  // initialize integration sum to zero
   ...
   for (int n=4; n<num_pts-1; n+=3)                // loop for even points
   {
     float x = x_min + interval * float(n-1);
...
{
   float quadra = 0.;
   double weight[10000], x[10000];             // for points and weights 
