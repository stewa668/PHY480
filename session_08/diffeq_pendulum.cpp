...
main (void)
{
  ...
  while (answer2 != 0)		// iterate until told to move on 
      ...
      while (answer != 0)	// iterate until told to move on 
	{
      ...
      my_filename_stream << "diffeq_pendulum_" << setprecision (2) << alpha
	<< ".dat";
      ...
      ofstream my_file;		// new filename (could have re-used my_file2)
      ...
      my_file << "# omega0=" << omega0 << ", alpha=" << alpha << endl;
      my_file << "# theta0=" << theta0 << ", theta_dot0=" << theta_dot0 <<
	endl;
      my_file << "# t_start=" << tmin << ", t_end=" << tmax << ", h=" << h <<
	endl;
      my_file << "#   t          x(t)                 v(t)       " << endl;

      if (tmin >= plot_min)
	{
	  my_file << tmin << "  " << scientific << setprecision (15)
      ...
      for (double t = tmin; t <= tmax; t += h)
	{
	  ...
	  if ((t >= plot_min) & (t <= plot_max))
		...
		{		// plot every plot_skip points 
		  my_file << t + h << " " << scientific << setprecision (15)
